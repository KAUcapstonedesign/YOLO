...
cam.set(cv2.CAP_PROP_FRAME_WIDTH, 1280) # 카메라 너비 설정
...
while True: # 무한 반복
    ...
    for(x,y,w,h) in faces:  # 얼굴이 검출되면
        cv2.rectangle(img, (x,y), (x+w,y+h), (255,255,255),1)   # 얼굴에 사각형 표시 / (x+w,y+h) : 얼굴 우측 하단 좌표, (255,255,255) : white, 1 : 선 두께
        ...
        cv2.putText(img,str(id), (x+5,y-5),font,1,(255,0,0),2)  # 이름 표시 / (x+5,y-5) : 얼굴 좌측 상단 좌표, (255,0,0) : blue, 2 : 선 두께
        cv2.putText(img,str(confidence), (x+5,y+h-5),font,1,(255,255,0),1)  # 정확도 표시 / (x+5,y+h-5) : 얼굴 좌측 하단 좌표, (255,255,0) : yellow, 2 : 선 두께
